...
public interface ProductRepository extends JpaRepository<Product, Long>, ProductSearch {
    ...
    @Query("select p, pi, avg(coalesce(r.grade,0)) from Product p " +
            "left join ProductImage pi " +
            "on (pi.product = p and pi.ord = (select min(pii.ord) " +
            "from ProductImage pii where pii.product = p)) " +
            "left join Review r " +
            "on r.product = p " +
            "group by p " +
            "order by avg(coalesce(r.grade,0)) desc, p.salesVolume desc " +
            "limit 6")
    ...
    @Query("select p, pi, avg(coalesce(r.grade,0)), count(distinct r) from Product p " +
            "left join ProductImage pi on pi.product = p left join Review r on r.product = p " +
